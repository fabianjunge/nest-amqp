...
describe('@Listen', () => {
  ...
  it('should set correctly the queue name and target data', () => {
    ...
    expect(metadata).toEqual(
      expect.objectContaining({ source: queueName, targetName: Test.name, callbackName: 'method1', connection: AMQP_DEFAULT_CONNECTION }),
    );
  });
  ...
  it('should set metadata correctly for `symbol` method', () => {
    ...
    expect(metadata).toEqual(
      expect.objectContaining({
        ...
        connection: AMQP_DEFAULT_CONNECTION,
      }),
    );
  });
});
